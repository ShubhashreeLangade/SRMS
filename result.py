...
class ResultClass:
    def __init__(self, root):
        ...
        btn_update = Button(self.root, text="Update", font=("times new roman", 15), bg="lightblue", activebackground="lightblue", cursor="hand2", command=self.update)
        btn_update.place(x=560, y=420, width=120, height=35)

        # Content window
        self.bg_img = Image.open("images/result.jpg")
        self.bg_img = self.bg_img.resize((500, 300), Image.LANCZOS)  # Use LANCZOS instead of ANTIALIAS
    ...
    def update(self):
        ...
        try:
            if self.var_name.get() == "":
                ...
                row = cur.fetchone()
                if row is None:
                    messagebox.showerror("Error", "No result found to update", parent=self.root)
                else:
                    per = (int(self.var_marks.get()) * 100) / int(self.var_full_marks.get())
